// =============================================================================
// Custom/GreekIslandTerrainV3
//
// Clean 4-layer slope-based terrain shader for URP.
// No contact-mask dependency — purely slope-driven layer blending + LOD.
//
...
// =============================================================================
Shader "Custom/GreekIslandTerrainV3"
{
    Properties
    {
        ...
        [Header(__________ Terrain Paint Map __________)]
        [Toggle] _UsePaintMap ("Enable Terrain Paint Blending", Float) = 0.0
        _Control ("Splat Control (set by Terrain)", 2D) = "red" {}
        _PaintBlend ("Paint vs Slope (0 = slope  1 = paint)", Range(0, 1)) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // =============================================================
            // Helper: sample all 4 layers at a given uniform scale
            // =============================================================
            struct LayerResult
            {
                half3 albedo;
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                // ---- Terrain paint map blending ----
                if (_UsePaintMap > 0.5)
                {
                    half4 ctrl = SAMPLE_TEXTURE2D(_Control, sampler_Control, input.terrainUV);
                    float pw1 = ctrl.r;
                    float pw2 = ctrl.g;
                    float pw3 = ctrl.b;
                    float pw4 = ctrl.a;
                    float pTotal = pw1 + pw2 + pw3 + pw4;
                    pTotal = max(pTotal, 0.001);
                    pw1 /= pTotal;
                    pw2 /= pTotal;
                    pw3 /= pTotal;
                    pw4 /= pTotal;
                    weight1 = lerp(weight1, pw1, _PaintBlend);
                    weight2 = lerp(weight2, pw2, _PaintBlend);
                    weight3 = lerp(weight3, pw3, _PaintBlend);
                    weight4 = lerp(weight4, pw4, _PaintBlend);
                }
